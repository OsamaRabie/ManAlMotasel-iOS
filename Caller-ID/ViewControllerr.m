    ...
    [[[_ref child:@"config"] child:userID] observeSingleEventOfType:FIRDataEventTypeValue withBlock:^(FIRDataSnapshot * _Nonnull snapshot) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^(){
            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                ...
                NSString* limit = @"70";
                NSString *encStr = [AESTool encryptData:[NSString stringWithFormat:@"%literm=%@&%liopt=%@&%lilimit=%@&%lislt=%li",(long)saltNumber,search,(long)saltNumber2,type,(long)saltNumber3,limit,(long)saltNumber4,(long)saltNumber5] withKey:@"9ONO45Rq1S68nLCm"];
    ...
    dispatch_async(dispatch_get_main_queue(), ^(){
        ...
        if (self->namesArray.count > 0)
        {
            ...
            if ([[NSUserDefaults standardUserDefaults] boolForKey:@"isNumberSearch"])
            {
                ...
                {
                    [self->headerLabel setText:[NSString stringWithFormat:@"We have more than 10 spam records.\nTop %lu:",(unsigned long)self->namesArray.count]];
                }else
                {
                    [self->headerLabel setText:[NSString stringWithFormat:@"لدينا أكثر من ١٠ تسجيلات، أعلى %lu:",(unsigned long)self->namesArray.count]];
                }
            }else
            {
                if([[[NSUserDefaults standardUserDefaults] stringForKey:@"lang"]isEqualToString:@"en"])
