...
//  spammers
//
//  Created by Osama Rabie on 13/01/2019.
...
- (void)beginRequestWithExtensionContext:(CXCallDirectoryExtensionContext *)context {
    ...
    NSString* block = [myDefaults stringForKey:@"block"];
    NSString* rblock = [myDefaults stringForKey:@"removeBlock"];
    
    if (context.isIncremental || (block && block.length > 0) || (rblock && rblock.length > 0)) {
...
- (void)addOrRemoveIncrementalBlockingPhoneNumbersToContext:(CXCallDirectoryExtensionContext *)context {
    ...
    CXCallDirectoryPhoneNumber phoneNumbersToAdd[] = { [block intValue] };
    ...
    for (NSUInteger index = 0; index < countOfPhoneNumbersToAdd; index += 1) {
        ...
        {
            [context addBlockingEntryWithNextSequentialPhoneNumber:phoneNumber];
        }else if(rblock && rblock.length > 0)
        {
